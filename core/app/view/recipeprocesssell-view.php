...
/// antes de proceder con lo que sigue vamos a verificar que:
		...
		foreach($cart as $c){
			...
			///
			$q = RecipeOperationData::getQYesF($c["product_id"]);
			if($c["q"]<=$q){
				if(isset($_POST["is_oficial"])){
				$qyf =RecipeOperationData::getQYesF($c["product_id"]); /// son los productos que puedo facturar
				if($c["q"]<=$qyf){
					$num_succ++;
				}else{
...
$_SESSION["errors"] = $errors;
	?>	
<script>
	window.location="index.php?view=recipe";
</script>
<?php
}
...
////////////////////
print "<script>window.location='index.php?view=recipeonesell&id=$s[1]';</script>";
		}
	}
}
