<?php
class RecipeOperationData {
	public static $tablename = "recipe";

	public function RecipeOperationData(){
		$this->name = "";
		$this->product_id = "";
		$this->piece_id = "";
		$this->q = "";
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (product_id,piece_id,q,user_id,created_at) ";
		$sql .= "value (\"$this->product_id\",\"$this->piece_id\",\"$this->q\",$this->user_id,$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto RecipeOperationData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set product_id=\"$this->product_id\",piece_id=\"$this->piece_id\",q=\"$this->q\" where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function getById($id){
		...
		return Model::one($query[0],new RecipeOperationData());
	}
	...
	public static function getAll(){
		...
		$query = Executor::doit($sql);
		return Model::many($query[0],new RecipeOperationData());
 ...
 $sql = "select * from ".self::$tablename." where date(created_at) >= \"$start\" and date(created_at) <= \"$end\" and product_id=$product order by created_at desc";
	...
	public static function getAllByPieceId($product_id){
		$sql = "select * from ".self::$tablename." where piece_id=$piece_id  order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new RecipeOperationData());


//	public static function getAllByProductIdCutIdOficial($product_id){
//		$sql = "select * from ".self::$tablename." where product_id=$product_id order by created_at desc";
//		return Model::many($query[0],new RecipeOperationData());
//	}
//
